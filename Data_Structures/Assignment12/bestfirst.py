# bestfirst.py CS2028 2016 Cheng
# Best first Algorithm with heap
...
def showGraph():
	...
	for vertex in vertexList:
		...
		if vertex.heapPosition == -2:
			t.color('red')
		elif vertex.heapPosition == -1:
			t.color('orange')
		else:
			t.color('gray')
		t.write(vertex.label,align="center",font=("Arial",7,"bold"))
	for edge in edgeList:
		if vertexList[edge.v1].heapPosition < -1 and vertexList[edge.v2].heapPosition < -1:
			t.color('red')
		else:
			t.color('gray')
		t.penup()
...
def distance(vertex1, vertex2):
	dx = (vertex1.x - vertex2.x) * 54
	dy = (vertex1.y - vertex2.y) * 69
	return math.sqrt(dx * dx + dy * dy)

def bestfirst(source, destination):
	vertexList[source].cost = distance(vertexList[source], vertexList[destination])
	...
	while heap.size > 0:
		...
		for edge in current.adjacentTo:
			...
			if adjacent.heapPosition >= 0 and adjacent.cost == 1000.0:
				adjacent.cost = distance(adjacent, vertexList[destination])
				adjacent.path = top
