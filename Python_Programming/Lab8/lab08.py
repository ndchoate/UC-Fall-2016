...
class Player(object):
    ...
    def go_to(self, direction):
        ...
        >>> bbc = Place('BCC', 'You are at Bearcat Cafe', [], [])
        >>> swift.add_exits([bcc])
        >>> bcc.add_exits([swift])
    ...
    def talk_to(self, person):
        ...
        place_characters = self.place.characters
        if person in place_characters:
            print(person + " says: " + place_characters[person].talk())
        else:
    ...
    def take(self, thing):
        ...
        elif self.place.things[thing] in self.backpack:
            print(thing + " is not here.")
        else:
            # If item is found, append the thing object from
            # Place's dictionary of things to player's backpack
            # and take the thing from the place's things
